...
export class ProcessingTranscriptionUseCase implements UseCaseInterface {
	...
	async handle(
		...
		if (transcriptionRequest && (await this.paymentGateway.checkPaymentSuccess(checkoutId))) {
					...
					if (audioSize > 20 * 1024 * 1024) {
						...
						for (let i = 0; i < numParts; i++) {
							...
							splittedFiles.push({
								...file,
								buffer: partBuffer,
								name: `${path.parse(file.name).name}_part${i + 1}.mp3`,
							});
						}
					} else {
			...
			const zip = new JSZip();
			await Promise.all(
				splittedFiles.map(async (file) => {
					console.log(file, 'file');
					await fs.writeFile(`/tmp/${file.name}`, file.buffer);
					const transcription = await this.transcriptorGateway.transcribeAudio(
						`/tmp/${file.name}`,
						file,
					);
					...
					await Promise.all([
						db
							.insertInto('transcription_request_item')
							.values({
								id: uuidv4(),
						...
						db
							...
							.set({ status: 'done' })
							.where('id', '=', transcriptionRequest.id)
							.execute(),
